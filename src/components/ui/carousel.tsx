...
function CarouselItem({ className, ...props }: React.ComponentProps<"div">) {
  ...
  React.useEffect(() => {
    ...
    const onSelect = () => {
      ...
      const currentNodeIndex = slideNodes.findIndex(node => node === itemRef.current)
      setIsSelected(currentNodeIndex === selectedIndex)
    }
    
    api.on("select", onSelect)
    onSelect() // Check initially
      ...
      {scrollSnaps.map((_, index) => (
        <button
          ...
          className={cn(
            ...
            index === selectedIndex ? "bg-gold w-8 h-2" : "bg-[#FFFFFF1A] size-2"
          )}
